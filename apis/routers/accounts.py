...
async def create_account(
    user_info: user_models.CreateAccount,
    ...
    try:
        ...
        user_data = db.query(UserModel).filter(UserModel.firebase_uid == firebase_uid).first()
        def_pkg = db.query(PackagesModel).filter(PackagesModel.package_name == "free").first()

        subscription = SubscriptionsModel(
            user_id=user_data.id,
            package_id=def_pkg.id,
            status="active",
            end_date=None,
        )
        db.add(subscription)
