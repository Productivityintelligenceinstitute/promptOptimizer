...
async def master_level_optimization_ws(websocket: WebSocket, db: Session = Depends(database.get_db)):
    await websocket.accept()
    handler = WebSocketTokenHandler(websocket)
    ...
    try:
        while True:
            ...
            await master_level_optimization_service(websocket, payload, db, handler)
