...
def _workflow_tool_guidance(job: ContextJobModel) -> str | None:
    ...
    if workflow == "contract_review":
        ...
        return (
            ...
            "from Retrieved Context — do not omit sections. The gateway merges full retrieved "
            "context automatically. When a canonical contract block is present, that full text "
            "is the source of truth for clause quotes, dates, caps, and notice periods.\n"
            "For contract URLs use source=url. Base your clause assessment on the tool's "
            "structured output plus retrieved evidence.\n"
            "Use retrieved contract and policy text as the source of truth. If a requested "
